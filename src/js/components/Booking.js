...
class Booking {
  ...
  render(element) {
    ...
    //TABLES wrapper
    this.dom.tablesWrapper = this.dom.wrapper.querySelector(
      select.booking.floorPlan
    );

    //STARTERS array of checkboxes
    this.dom.startersWrapper =
      this.dom.wrapper.querySelector('.booking-options').children[2];
    ...
    //booking options
    this.dom.bookingOptions =
      this.dom.wrapper.querySelector('.booking-options');
    ...
    //PHONE and ADDRESS
    this.dom.orderConfirmation = this.dom.wrapper.querySelector(
  ...
  getData() {
    ...
    Promise.all([
      ...
      .then(function (allResponses) {
        //function receive responses from all the fetches
  ...
  sendOrder() {
    ...
    const phoneElement = thisBooking.dom.orderConfirmation.querySelector(
      select.cart.phone
    );
    const addressElement = thisBooking.dom.orderConfirmation.querySelector(
      select.cart.address
    );
    ...
    for (let starterKey in startersObj) {
      startersObj[starterKey].addEventListener('click', function (event) {
        if (
          event.target.getAttribute('value') == starterKey &&
    ...
    thisBooking.dom.bookingForm.addEventListener('submit', function () {
      ...
      payload = {
        ...
        hour: thisBooking.formatHour(thisBooking.hour),
